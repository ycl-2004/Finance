...
本站目前是 **專業顧問機構感的中文金融知識中心**，不是行銷 landing page，也不是單純文檔站。
...
| --- | --- | --- | --- |
| `--bg` | `#f7f6f2` | 全站背景 | 暖白、柔和、閱讀友好 |
| `--surface` | `#ffffff` | 卡片、正文、面板 | 乾淨、正式 |
| `--surface-muted` | `#f0eee7` | hover、弱背景、表頭 | 溫和分區 |
| `--text` | `#1d2528` | 主文字 | 接近黑色但不刺眼 |
| `--muted` | `#647174` | 輔助文字 | 冷灰、穩定 |
| `--border` | `#dedbd2` | 邊框 | 低對比、分隔資訊 |
| `--accent` | `#0f6b63` | 主強調色 | 沉穩藍綠 |
| `--accent-strong` | `#084a45` | 主按鈕、重要互動 | 更深、更可信 |
| `--accent-soft` | `#e3f2ef` | tag / 柔和強調背景 | 不刺眼的提示 |
| `--warning` | `#9d5a00` | 警示文字 | 合規提醒 |
| `--warning-soft` | `#fff4df` | 教育邊界提示背景 | 溫和但明顯 |
| `--danger` | `#9b2f2f` | 高風險提示預留 | 嚴肅風險 |

### 色彩性格

整體是 **暖白 + 中性灰 + 沉穩藍綠 + 少量琥珀警示**。
